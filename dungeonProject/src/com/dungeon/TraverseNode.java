...
public class TraverseNode {
  ...
  public DungeonAndLeftOverPath generateDungeonAndLeftOverPath(ArrayList<Path> potentialPaths) {
    ...
    for (int i = 0; i < potentialPaths.size(); i++) {
      ...
      // Start & End (both) node of the path should be matched
      ...
      // "+startEdge.getY()+" >> end: "+endEdge.getX()+", "+endEdge.getY()+" >> is leftover:
      // "+isLeftOverPath);
      ...
      if (isLeftOverPath) {
        ...
        //Check this path is joint of any existing two sets by checking Start Edge in one set
        // & End node in another set
        //If so then merge those 2 set as one
        int firstSetIndex = -1;
        ...
        if (firstSetIndex != -1 && secondSetIndex != -1 && firstSetIndex != secondSetIndex) {
          ...
          //replace the new temp. maze into actual maze, as merged set should be primary
        ...
        for (int j = 0; j < maze.size(); j++) {
          ...
          if (isAddedExistingSet) {
            //Since the path start node is end node of existing set one of the page
            //this can be added existing set
